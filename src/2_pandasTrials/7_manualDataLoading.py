# requires data_2d.csv

import numpy as np

x = [] # Empty list to store values

# That csv file is 100 lines with three values each line.
# We need a matrix.
# using map(fun,iter)
...
for line in open("data_2d.csv"):
    row = line.split(',')  # each line is split row which contains each string between commas
    sample = map(float, row)  # row is string, it is cast into float using float() in map
    x.append(sample)  # x will be a list of lists

# Each line is split and a row list is made which contains values of one row as string
# each item in row which is a string is cast to float and saved in a list called sample as map() returns a list
# The list sample gets appended to list x
# finally after for loop is done, x will be a list of lists where inner list contains three floats

numpyArray = np.array(x)
print(numpyArray)  # output will only be lists with <map object at 0xq956245897364>
...
# Actual workflow at ipython shown below
#
# deltonmyalil@ubuntu:~/dataSciencePrerequisiteStack/src/2_pandasTrials$ ipython
